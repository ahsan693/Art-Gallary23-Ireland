      ...
      <ul className="caption mt-[16px] flex flex-col gap-[12px] text-muted sm:mt-[24px] sm:gap-[14px]">
        {items.map((item, index) => {
          ...
          return <li key={label} className="flex max-w-[260px] items-start gap-[8px] sm:max-w-[200px]">
            ...
            {contactHref ? <Link href={contactHref} className={isHighlight ? "font-bold text-white hover:text-white" : "hover:text-white"}>{label}</Link> : <span className={isHighlight ? "font-bold text-white" : ""}>{label}</span>}
          </li>;
        })}
      </ul>
    </div>
